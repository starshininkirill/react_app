export const retranslate = function (word) {
   const dict = [
     {
       'en': 'cook',
       'ru': 'Повар'
     ...
     {
      'en': '-name',
      'ru': 'По имени в обратном порядке'
     },
...
export const getUserOptions = function(users){
  let roleOptions = []
  let isArchiveOptions = []
  users.forEach(function(user){
  if ( !roleOptions.includes(user.role)) {
        roleOptions.push(user.role)
    }
  if (!isArchiveOptions.includes(String(user.isArchive))) {
    isArchiveOptions.push(String( user.isArchive))
  }
  })

  return {roleOptions, isArchiveOptions};
}

export const sortUsers = function(users ,method){
  let filtredUsers = users
  if (method == 'name') {
    filtredUsers = users.sort(function(a, b){
  ...
  if (method == 'date') {
    filtredUsers = users.sort(function(a, b){
      let dateA = new Date(a.birthday.split('.').reverse().join('-')).getTime()
      let dateB = new Date(b.birthday.split('.').reverse().join('-')).getTime()
